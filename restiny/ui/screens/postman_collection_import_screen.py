...
class _IncompatibleVersionError(Exception):
    pass


class _ImportFailError(Exception):
    pass
...
class PostmanCollectionImportScreen(ModalScreen):
    ...
    def _on_confirm(self) -> None:
        try:
            self._import()
        except _IncompatibleVersionError:
            self.notify(
                'Collection version incompatible (only v2.1 is supported)',
                severity='error',
            )
            return
        except _ImportFailError:
            self.notify('Failed to import collection', severity='error')
            return

        self.notify(
            message='Postman collection imported', severity='information'
        )
        self.dismiss(result=True)

    def _import(self) -> None:
        collection = json.loads(self.collection_file_chooser.path.read_text())

        if 'v2.1' not in collection['info']['schema']:
            raise _IncompatibleVersionError()
