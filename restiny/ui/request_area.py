...
class RequestArea(Static):
    ...
    def _on_paste_auth(self, message: Button.Pressed) -> None:
        ...
        if self._auth_clipboard['mode'] == AuthMode.BASIC:
            self.auth_basic_username = self._auth_clipboard[
                'basic_username'
            ]
            self.auth_basic_password = self._auth_clipboard[
                'basic_password'
            ]
        elif self._auth_clipboard['mode'] == AuthMode.BEARER:
            self.auth_bearer_token = self._auth_clipboard[
                'bearer_token'
            ]
        elif self._auth_clipboard['mode'] == AuthMode.API_KEY:
            self.auth_api_key_where = self._auth_clipboard[
                'api_key_where'
            ]
            self.auth_api_key_key = self._auth_clipboard[
                'api_key_key'
            ]
            self.auth_api_key_value = self._auth_clipboard[
                'api_key_value'
            ]
        elif self._auth_clipboard['mode'] == AuthMode.DIGEST:
            self.auth_digest_username = self._auth_clipboard[
                'digest_username'
            ]
            self.auth_digest_password = self._auth_clipboard[
                'digest_password'
            ]
